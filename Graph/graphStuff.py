...
def create_graph(edge_index, edge_attr, node_texts, node_types, module_numbers):
    edge_index = torch.tensor(edge_index, dtype=torch.long).t().contiguous()

    data = Data(edge_index=edge_index, edge_attr=edge_attr)
    data.edge_attr = edge_attr
    data.node_texts = node_texts  # Speichern der node_texts als Liste von Strings
    data.node_types = node_types  # Speichern der node_types als Liste von Strings
    data.module_numbers = module_numbers  # Speichern der module_numbers als Liste von Integers
    return data
...
def create_node_base_sentences(sentences):
    ...
    for sentence, module_number in sentences:
        node_texts.append(sentence)  # Speichern des Satzinhalts
        node_types.append("Satz")  # Typ für jeden Satzknoten
        module_numbers.append(module_number)  # Speichern der Modulnummer
...
def create_initial_graph(all_edges, all_edge_attrs, node_texts, node_types, module_numbers, device):
    ...
    graph_data = create_graph(all_edges, all_edge_attrs, node_texts, node_types, module_numbers).to(device)
    return graph_data
