...
def create_graph(node_features, edge_index, edge_attr):
...
def add_sentence_to_graph(graph_data, all_nodes, sentence_start_indices, new_entities, new_relations, new_edge_attr):
    ...
    graph_data.edge_attr = torch.cat([graph_data.edge_attr, new_edge_attr], dim=0)
    ...
    if len(sentence_start_indices) > 1:
        ...
        graph_data.edge_attr = torch.cat([graph_data.edge_attr, torch.tensor([1.0], dtype=torch.float)], dim=0)
...
if __name__ == "__main__":
    ...
    relations = [(0, 1)]
    edge_attr = [0.9]  # Beispiel Ähnlichkeitswert

    graph_data = create_graph(entities, relations, edge_attr)
    print("Initialer Graph:", graph_data)

    new_entities = [[3], [4]]
    new_relations = [(0, 1)]
    new_edge_attr = [0.8]  # Beispiel Ähnlichkeitswert

    graph_data, all_nodes, sentence_start_indices = add_sentence_to_graph(graph_data, entities, [0], new_entities,
                                                                          new_relations, new_edge_attr)
