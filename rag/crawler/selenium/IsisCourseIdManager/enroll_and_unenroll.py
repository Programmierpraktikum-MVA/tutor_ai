...
from paths import ISIS_ALL_ACCESSIBLE_COURSES_FILE, ISIS_MY_COURSE_IDS_FILE
...
def enroll_and_unenroll():
    ...
    with open(ISIS_ALL_ACCESSIBLE_COURSES_FILE, encoding="utf-8") as file:
        all_accessible_courses = json.load(file)
    if isinstance(all_accessible_courses, dict):
        all_accessible_courses = all_accessible_courses.get("course_ids", [])

    # Get all your courses
    get_all_your_course_ids.get_all_course_id(driver)
    # Open all your course IDs
    with open(ISIS_MY_COURSE_IDS_FILE, encoding="utf-8") as file:
