...
from paths import ISIS_UPLOAD_LOG_FILE, ensure_dir

# Configure logging to write to a file
ensure_dir(ISIS_UPLOAD_LOG_FILE.parent)
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler(ISIS_UPLOAD_LOG_FILE),
        logging.StreamHandler()  # This will still print to the console
    ]
)

def load_config():
    config_override = os.environ.get("DROPBOX_CONFIG")
    config_path = Path(config_override) if config_override else Path(__file__).resolve().parent / "config.json"
    if not config_path.exists():
        raise FileNotFoundError(f"Dropbox config not found at {config_path}")
